...
The Cloud Foundry Data Flow Server is Spring Boot application available for https://cloud.spring.io/spring-cloud-dataflow/#platform-implementations/[download] or you can https://github.com/spring-cloud/spring-cloud-dataflow-server-cloudfoundry[build] it yourself.
...
. Connect the `shell` with `server` running on Cloud Foundry, e.g., `https://dataflow-server.app.io`
+
```
...
+
```
server-unknown:>dataflow config server https://dataflow-server.app.io
Successfully targeted https://dataflow-server.app.io
dataflow:>
```
