...
Below we will present the steps to prep, configure the demo of Spring Cloud Data Flow's `Local` server integration with `InfluxDB`. For other deployment environment, such as `Cloud Foundry` or `Kubernetes`, addition configurations might be required.
...
. Register `time` and `log` applications pre-build `io.micrometer:micrometer-registry-influx`. Coming to next version of https://start-scs.cfapps.io/[SCSt App Initializr] is support adding Micrometer registry dependencies as well.
+
```bash
...
```
The `app.*.management.metrics.export.influx.db=myinfluxdb` instructs the `time2` and `log2` apps to use the `myifluxdb` database (created automatically). 
+
