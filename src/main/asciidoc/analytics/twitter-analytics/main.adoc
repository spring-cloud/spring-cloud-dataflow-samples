...
+
```
dataflow:>stream create tagcount  --definition ":tweets.twitterstream > counter --name=hashtags --counter.tag.expression.hastag=#jsonPath(payload,'$.entities.hashtags[*].text')" --deploy
