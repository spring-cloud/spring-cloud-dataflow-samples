...
+
Notice how we use the Kubernetes Persistent Volume Claim(PVC) resource that we created earlier. 
...
+
NOTE: The deployment property `deployer.*.kubernetes.bootMajorVersion=2` tells the deployer to configure the liveness and readiness probes for Spring Boot 2.x applications, as Spring Boot actuator endpoints change in version 2. This will likely become the default in a future SCDF release.
+
[source,console,options=nowrap]
----
...
scdf-server-64fb996ffb-dmwpj                 1/1     Running     4          1d
----
+
